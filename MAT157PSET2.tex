...
Case 1: $b_n - a_n = 0$. If this is the case then the set $[a_n, b_n]$ is just the singleton set $\{ a_n \}$. We know that $a_n$ is real because it has a supremum, namely $Sup( \{ a_n \} ) = a_n$. In this case we can say that $\bigcap_{n \in \mathbb{N}}I_n = \{ a_n \}$ because $I_n$ is the smallest possible subset of $I_1 \cap I_2 \cap \cdots \cap I_{n-1}$ such that $I_n$ is non-empty, making all subsequent $I_m = I_n$ where $m>n$; thus, it is non-empty and closed. \\
